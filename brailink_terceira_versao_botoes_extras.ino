...
void setup() {
  // Configuração dos botões extras
  pinMode(enterBtn, INPUT_PULLUP);
  pinMode(espacoBtn, INPUT_PULLUP);
  pinMode(retrocessoBtn, INPUT_PULLUP);
  pinMode(setaCima, INPUT_PULLUP);
  pinMode(setaBaixo, INPUT_PULLUP);
  pinMode(setaEsquerda, INPUT_PULLUP);
  pinMode(setaDireita, INPUT_PULLUP);
  pinMode(volumeMais, INPUT_PULLUP);
  pinMode(volumeMenos, INPUT_PULLUP);
  pinMode(imprimirBtn, INPUT_PULLUP);
  pinMode(navegadorBtn, INPUT_PULLUP);
...
void loop() {
  // Leitura dos botões extras
  if (digitalRead(espacoBtn) == LOW) {
  ...
  if (digitalRead(retrocessoBtn) == LOW) {
    Serial.println("[RETROCESSO]");
    delay(300);
  }
  if (digitalRead(setaCima) == LOW) {
    Serial.println("[↑]");
    delay(300);
  }
  if (digitalRead(setaBaixo) == LOW) {
    Serial.println("[↓]");
    delay(300);
  }
  if (digitalRead(setaEsquerda) == LOW) {
    Serial.println("[←]");
    delay(300);
  }
  if (digitalRead(setaDireita) == LOW) {
    Serial.println("[→]");
    delay(300);
  }
  if (digitalRead(volumeMais) == LOW) {
    Serial.println("[VOLUME +]");
    delay(300);
  }
  if (digitalRead(volumeMenos) == LOW) {
    Serial.println("[VOLUME -]");
    delay(300);
  }
  if (digitalRead(imprimirBtn) == LOW) {
    Serial.println("[IMPRIMIR]");
    delay(300);
  }
  if (digitalRead(navegadorBtn) == LOW) {
    Serial.println("[NAVEGADOR]");
    delay(300);
  }
